  ...
  if (count === 0) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <Link href="/urunler">
          <Button className="bg-green-700 hover:bg-green-800">Ürünlere Git</Button>
        </Link>
      </div>
    )
  }
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="grid md:grid-cols-3 gap-8">
            ...
            return (
              <div key={item.product.id} className="bg-white border rounded-xl p-4 flex gap-4">
                ...
                <div className="flex-1">
                  <Link href={`/urunler/${item.product.slug}`}>
                    <h3 className="font-medium text-gray-800 hover:text-green-700 text-sm">{item.product.name}</h3>
                  </Link>
                  {item.product.sku && <p className="text-xs text-gray-400">SKU: {item.product.sku}</p>}
                  <p className="text-green-700 font-bold mt-1">{formatPrice(item.product.price)}</p>
        ...
        <div className="bg-white border rounded-xl p-5 h-fit sticky top-20">
          ...
          <Link href="/odeme">
            <Button className="w-full bg-green-700 hover:bg-green-800 mb-3" size="lg">
              Ödemeye Geç
            </Button>
          </Link>
